...
plugin.fillDetails = function(d) {
  ...
  $('#torrentDetails #status td:last').text(theWebUI.getStatusIcon(d)[1] + ' ').append('<button class="btn btn-default btn-sm" type="button" onclick="mobile.recheck();" title="Recheck">↻</button>');
...
plugin.drawFiles = function(p) {
  ...
  for (var name in dir.container) {
    ...
    if (dir.container[name].directory) {
      ...
      '📄&nbsp;' + name + '</a><div style="display:none;" id="' + idName + '">' +
      '<table class="table table-striped"><tbody>' +
...
plugin.update = function(singleUpdate) {
  ...
  function(data) {
    ...
    var plabelMap =
      ...
      theWebUI.categoryList.panelLabelAttribs &&
      theWebUI.categoryList.panelLabelAttribs.plabel;
    var noLabelCount = 0;
    if (plabelMap) {
      var noLabelEntry = plabelMap.get("-_-_-nlb-_-_-");
      if (noLabelEntry && noLabelEntry.count != null) {
        noLabelCount = noLabelEntry.count;
      }
    }
    var labelsHtml =
    ...
    if (plabelMap) {
      for (var l of plabelMap.keys()) {
        if (l.startsWith("clabel__")) {
          var labelProper = l.replace("clabel__", "");
          if (plugin.labelIds[labelProper] == undefined) {
            plugin.labelIds[labelProper] = nextLabelId++;
          }

          var lblInfo = plabelMap.get(l);
          var lblCount = lblInfo && lblInfo.count != null ? lblInfo.count : 0;
          labelsHtml +=
            '<li><a href="javascript://void();" onclick="mobile.filter(mobile.statusFilter.label, this, \'' +
            labelProper +
            '\');">' +
